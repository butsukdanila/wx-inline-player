...
const f_glue = path.join(__dirname, 'glue.js');
const f_glue_content = fs.readFileSync(f_glue).toString();

const f_codec = path.join(argv[2], `${argv[3]}.js`);
const f_codec_content = fs.readFileSync(f_codec).toString();

const f_codec_prod = path.join(argv[2], `${argv[3]}.prod.js`)
let f_codec_prod_content = `
...
fs.writeFileSync(
  f_codec_prod,
  uglify.minify(f_codec_prod_content).code
);
